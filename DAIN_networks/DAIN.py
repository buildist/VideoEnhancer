...
from DAIN_extensions.FilterInterpolation import  FilterInterpolationModule
from DAIN_extensions.FlowProjection import  FlowProjectionModule #,FlowFillholeModule
from DAIN_extensions.DepthFlowProjection import DepthFlowProjectionModule

from DAIN_Stack import Stack

import PWCNet
import S2D_models
import DAIN_Resblock
import DAIN_MegaDepth
import time
...
class DAIN(torch.nn.Module):
    def __init__(self,
        ...
        self.rectifyNet = DAIN_Resblock.__dict__['MultipleBasicBlock_4'](3 + 3 + 3 +2*1+ 2*2 +16*2+ 2 * self.ctx_ch,128)
        ...
        if self.training:
            self.depthNet=DAIN_MegaDepth.__dict__['HourGlass']("MegaDepth/checkpoints/test_local/best_generalization_net_G.pth")
        else:
            self.depthNet=DAIN_MegaDepth.__dict__['HourGlass']()
